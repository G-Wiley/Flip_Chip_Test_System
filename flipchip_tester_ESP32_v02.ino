// Flip Chip Tester, port of Stearns Tester code to ESP32 on Arduino
//
...
int read_test(/*File* file_test*/)    /* returns 0 if no error    */
{
    ...
    char            rtbuffer[MAX_STRING]; // read test or raw test buffer, stores one line from TST file
    char            character;
    char            dir_character;
...
void setup() {
  ...
  print("\r\n");
  print("BOOT - PDP-8 Flip Chip card tester, Arduino ESP32\r\n");
...
void loop() {
  ...
  // https://forum.arduino.cc/t/where-goes-the-esp32-memory/649117
  Serial.println("     free heap:      " + String(ESP.getFreeHeap()));
  Serial.println("     min free heap:  " + String(ESP.getMinFreeHeap()));
  Serial.println("     heap size:      " + String(ESP.getHeapSize()));
  Serial.println("     max alloc heap: " + String(ESP.getMaxAllocHeap()));
    ...
    for (;;)      /* will break on success  */
    {
      ...
      while(true){
        if(Serial.available()>0){
          ...
          if((incomingbyte != 0x7f) || (theinputstring.length() > 0))
            Serial.print(incomingbyte);
          if((incomingbyte == '\n') || (incomingbyte == '\r')) break;
          if(incomingbyte == 0x7f){
            if(theinputstring.length() > 0)
              theinputstring.remove(theinputstring.length()-1);
          }
          else theinputstring = theinputstring + incomingbyte;
        }
      }
      Serial.println("\nentered: " + String(theinputstring));
      Serial.println("length=" + String(theinputstring.length()));
